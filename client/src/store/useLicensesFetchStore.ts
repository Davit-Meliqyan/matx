...
export const useLicenseStore = create<LicenseStore>((set, get) => ({
  ...
  fetchItems: async (section, params) => {
    ...
    const res = await fetch(`/api/${section}/license${query}`); // GET без заголовков
    if (!res.ok) {
  ...
  fetchItemById: async (section, id, params) => {
    const query = buildQuery(params);
    const res = await fetch(`/api/${section}/license/${id}${query}`); // GET без заголовков
  ...
  createItem: async (section, item, params) => {
    const query = buildQuery(params);
    const res = await fetch(`/api/${section}/license${query}`, {
      method: "POST",
  ...
  updateItem: async (section, item, params) => {
    const query = buildQuery(params);
    const res = await fetch(`/api/${section}/license/${item.id}${query}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(item),
    });
  ...
  deleteItem: async (section, id, params) => {
    const query = buildQuery(params);
    const res = await fetch(`/api/${section}/license/${id}${query}`, {
      method: "DELETE",
    });
  ...
  uploadFile: async (section: string, id: string, files: File[]) => {
    ...
    const res = await fetch(`/api/${section}/license/${id}/upload`, {
      method: "POST",
      body: formData,
    });
  ...
  removeFile: async (section, id, fileName) => {
    const res = await fetch(`/api/${section}/licenses/${id}/${fileName}`, {
      method: "DELETE",
    });
